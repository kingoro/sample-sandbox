...
```sh
cargo +nightly-2026-06-06 fuzz run operation_sequence \
  --fuzz-dir memory-buffer/fuzz \
  memory-buffer/fuzz/artifacts/operation_sequence/{artifact}
```
...
- `memory-buffer/fuzz/corpus/operation_sequence/`: 到達経路を増やした入力の学習データ
- `memory-buffer/fuzz/artifacts/operation_sequence/`: crashやassertion failureを再現する入力
- `memory-buffer/fuzz/target/`: fuzz targetのbuild成果物
