...
組み込みLinux製品のApplication Logを扱う、製品・domain非依存のC Foundation。
...
Log FoundationはRecordを生成・蓄積・読出しするところまでを担当する。dumpしたRecordを
どこへ送るかは製品側の保守機能が決定する。

## 資料

- [実行可能サンプル3パターン](examples/README.md)
- [初心者向け はじめてのLog Foundation](README_BEGINNER.md)
...
```sh
make foundation-log-test
make foundation-log-examples
make foundation-log-static-analysis
make foundation-log-fuzz-smoke
make check
```
