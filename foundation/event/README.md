...
- trace recordをLog Foundationへ出力するadapter
...
| 項目 | 状態 | 内容 |
| --- | --- | --- |
| 基本型・エラー体系 | 一部 | Event Foundation内の型とresult codeのみ |
...
| ログ・状態遷移trace | 実装済み | Event trace hook、State Machine自動遷移trace、Log Foundation adapter |
...
- [初心者向け はじめてのEvent Foundation](README_BEGINNER.md)
...
```sh
make foundation-event-test
make foundation-event-examples
make foundation-static-analysis
make foundation-event-fuzz-smoke
```

`make foundation-event-test`はAPI単体テストに加え、Queue、Dispatcher、State Machine、
