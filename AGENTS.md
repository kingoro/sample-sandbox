...
- 言語、Foundation、moduleごとに独自の品質閾値を追加しない。
...
- 外部公開入口は`foundation/event/include/utility_event.h`とする。
- 実装、header、単体テストは責務単位で分割する。
- Event Foundation変更後は`make foundation-event-test`と
  `make foundation-static-analysis`を実行する。
- 完了前に`make check`を実行する。

## Log Foundation

- 外部公開入口は`foundation/log/include/utility_log.h`とする。
- EEPROM製品Log、通信、USB/UART出力をApplication Log基盤へ混在させない。
- Log RecordはRAM Ring内でmessageとmetadataを所有し、呼出元pointerへ依存させない。
- 実装、header、単体テストは責務単位で分割する。
- Log Foundation変更後は`make foundation-log-test`と
  `make foundation-log-static-analysis`を実行する。
- 完了前に`make check`を実行する。
